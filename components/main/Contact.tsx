    ...
    return (
        <section id="contact" className={`bg-orange-50 bg-section-light dark:bg-zinc-900 dark:bg-section-dark text-black dark:text-white p-4 min-h-125 flex flex-col items-center`}>
            ...
            {loading && <div className="flex justify-center items-center p-3">
                <Loader color={loaderColor} size={40} />
            </div>}

            <form id="contactForm" className={`bg-orange-100 dark:bg-zinc-800 text-base lg:text-lg my-10 min-h-96 rounded-lg w-11/12 lg:w-2/6 p-9 flex flex-col `} onSubmit = {handleSubmit}> 
                <label htmlFor="name" className="my-2">Name</label>
                <input type="text" placeholder="John Doe" name="name" id="name" className={`rounded-lg p-3 h-10 bg-orange-300 placeholder:text-gray-600 dark:bg-zinc-900 dark:placeholder:text-gray-300`} required />
                ...
                <button className={`rounded-lg p-3 mt-5 mx-auto bg-orange-300 placeholder:text-gray-600 dark:bg-zinc-900 dark:placeholder:text-gray-300 flex gap-2 items-center text-base lg:text-lg`} type="submit">Send Message <i className="fa-solid fa-paper-plane" aria-hidden="true"></i></button>
            </form>
        </section>
    )
}
