...
function Project({ children, title, src, github } : { children: ReactNode, title: string, src: string, github: string }) {
    ...
    return (
        <div className="min-h-96 m-6 grid grid-cols-1 lg:grid-cols-2 items-center justify-items-center text-base lg:text-lg">
            {(num % 2 == 0) && <video className="rounded-lg m-5" controls>
            ...
            {(num % 2 == 1) && <video className="rounded-lg m-5" controls>
                <source src={src} type="video/mp4" />
                Your browser does not support the video tag.
            </video>}
        </div>
